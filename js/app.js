...
let num_2, respuesta_elegida, respuesta_correcta;
...
function generar_pregunta() {
    num_2 = Math.random() * (12 - nivel) + nivel;
    num_2 = Math.floor(num_2);
    if (num_2 != 1) {
        texto_pregunta.innerText = nivel + " x " + Math.floor(num_2) + " = ?";

    } else {
        texto_pregunta.innerText = nivel + " x " + Math.floor(num_2 + 1) + " = ?";
    }
...
function generar_opciones() {
    ...
    for (let i = 0; i < 4; i++) {
        if (opciones[i] == 0) {
            opciones[i] = Math.floor(Math.random() * (num_2 - (num_2 * 2)) + (num_2 * 2));
